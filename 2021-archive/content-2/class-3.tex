...
{\bf 一个重要结论}：考虑$m$阶Jordan块$J_m(0)$, 容易看到当$1\leqslant k \leqslant m-1$时，$J_m(0)^k$是一个第$(i, i+k)$位（或者第第$(i+k, i)$位），$i=1,\ldots m-k$, 元素值为$1$, 其余位置为$0$的方阵，因此
...
\scalebox{0.9}{
  \begin{tikzpicture}
  \draw[gray, ->] (-1, 0) -- (4, 0) node[right] {$k$};
  \draw[gray, ->] (0, -1) -- (0, 3) node[above] {$rank$};
  \draw[domain=0:2, smooth, variable=\x, blue] plot ({\x}, {2-\x});
  \draw[domain=2:4, smooth, variable=\x, blue]  plot ({\x}, {0});
  \node [left] at (0, 2) {$m$};
  \node [below] at (2, 0) {$m$};
\end{tikzpicture}
}
...
\scalebox{0.9}{
  ...
  \draw[gray, ->] (-1, 0) -- (4, 0) node[right] {$k$};
  \draw[gray, ->] (0, -1) -- (0, 3) node[above] {$rank'$};
  \draw[domain=0:2, smooth, variable=\x, blue] plot ({\x}, {1});
  \draw[domain=2:4, smooth, variable=\x, blue]  plot ({\x}, {0});
  \draw[domain=0:1, smooth, variable=\y, blue, dashed]  plot ({2}, {\y});
  \node [left] at (0, 1) {$1$};
  \node [below] at (2, 0) {$m$};
\end{tikzpicture}
}
...
\scalebox{0.9}{
  ...
  \draw[gray, ->] (-1, 0) -- (4, 0) node[right] {$k$};
  \draw[gray, ->] (0, -1) -- (0, 3) node[above] {$rank''$};
  \draw[domain=0:2, smooth, variable=\x, blue] plot ({\x}, {0});
  \draw[domain=2:4, smooth, variable=\x, blue]  plot ({\x}, {0});
  \draw[domain=0:1, smooth, variable=\y, blue, dashed]  plot ({2}, {\y});
  \draw[domain=0:2, smooth, variable=\x, blue, dashed]  plot ({\x}, {1});
  \node [left] at (0, 1) {$1$};
  \node [below] at (2, 0) {$m$};
  \node [scale=0.3, circle, draw=blue, fill=white] at (2, 0) {};
  \node [scale=0.3, circle, fill, blue] at (2, 1) {};
\end{tikzpicture}
}
\endminipage
\end{figure}
